#!/bin/bash
#SBATCH --job-name=leflow_uncertainty_eval
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
#SBATCH --output=leflow_uncertainty_eval_reacher_%j.out
#SBATCH --error=leflow_uncertainty_eval_reacher_%j.err

set -euo pipefail

# Explicit checkpoint and threshold support both training methods and run names.
if [[ $# -ne 2 ]]; then
  echo "Usage: sbatch jobs/eval_latent_uncertainty_planner.sh <checkpoint.pt> <threshold>" >&2
  exit 2
fi
PLANNER_CHECKPOINT="$1"
UNCERTAINTY_THRESHOLD="$2"
UNCERTAINTY_SOURCE="${UNCERTAINTY_SOURCE:-auto}"
MAX_RESAMPLE_ROUNDS="${MAX_RESAMPLE_ROUNDS:-100}"
...
RESULTS_DIR="${RESULTS_DIR:-${RESULTS_ROOT}/reacher/uncertainty/run_${RUN_ID}}"
...
if [[ ! -f "${PLANNER_CHECKPOINT}" ]]; then
  echo "Missing uncertainty planner checkpoint: ${PLANNER_CHECKPOINT}" >&2
  exit 1
fi
...
echo "=== Snellius LeFlow Reacher uncertainty evaluation started ==="
echo "node=$(hostname)"
echo "repo=${REPO_DIR}"
echo "checkpoint=${PLANNER_CHECKPOINT}"
echo "uncertainty_source=${UNCERTAINTY_SOURCE}"
echo "uncertainty_threshold=${UNCERTAINTY_THRESHOLD}"
echo "max_resample_rounds=${MAX_RESAMPLE_ROUNDS}"
...
for seed in "${SEEDS[@]}"; do
  ...
  srun --unbuffered python eval_with_uncertainty.py --config-name=reacher.yaml \
    solver=latent_flow_uncertainty \
    ...
    solver.uncertainty_threshold="${UNCERTAINTY_THRESHOLD}" \
    solver.uncertainty_source="${UNCERTAINTY_SOURCE}" \
    solver.max_resample_rounds="${MAX_RESAMPLE_ROUNDS}" \
...
echo "=== Snellius LeFlow Reacher uncertainty evaluation finished ==="
